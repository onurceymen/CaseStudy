...
namespace CaseStudyData.Context
{
    public static class SeedData
    {
        public static async Task Initialize(IServiceProvider serviceProvider)
        {
            ...
            // Veritabanında rollerin var olup olmadığını kontrol edin
            if (!context.Roles.Any())
            {
                context.Roles.AddRange(
                    new Role { RoleName = "Admin", CreatedAt = DateTime.Now },
                    new Role { RoleName = "Seller", CreatedAt = DateTime.Now },
                    new Role { RoleName = "Buyer", CreatedAt = DateTime.Now }
                );
                ...
                //        //.RuleFor(r => r.Id, f => 0)
                //        .RuleFor(r => r.RoleName, f => rolesArray[f.IndexFaker])
